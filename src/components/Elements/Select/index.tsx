...
export const Select: FC<SelectProps> = ({
  ...
  const itemSelectedLabel = itemSelected?.title || itemSelected?.name || placeholder || 'Selecione';

  const toggleList = () => {
    if (!disabled) setOpenList((e) => !e);
  };

  const onChangeInput = (e: { target: { value: string } }) => setInputSearch(e.target.value);
  ...
  return (
    <S.Wrapper disabled={disabled} open={openList} loading={loading} ref={node as React.RefObject<HTMLDivElement>}>
        ...
        <S.List>
          {fetchOptions && (
            <S.Item className="select-search">
              <input
                value={inputSearch}
                ref={nodeInput as React.RefObject<HTMLInputElement>}
                onChange={(e) => onChangeInput(e)}
              />
              <button type="button" className="select-search" onClick={() => fetchOptions(inputSearch)}>
          ...
          {options.map((item) => (
            <S.Item key={item.id} value={item.value} selected={item.selected} onClick={() => selectedItem(item.value)}>
